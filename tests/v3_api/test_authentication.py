...
AD_PERMISSION_DENIED_CODE = 401    # Unauthorized
LDAP_PERMISSION_DENIED_CODE = 403  # Forbidden
...
CATTLE_AUTH_TEST_AND_APPLY_URL = CATTLE_AUTH_PROVIDER_URL + "?action=testAndApply"
...
def test_disable_and_enable_auth_set_access_control_required():
    access_mode = "required"
    validate_access_control_disable_auth(access_mode)
...
#by default nestedgroup is disabled for ad and openldap
def test_enable_nestedgroup_for_openldap_set_access_control_required():
    access_mode = "required"
...
#
#
# def test_enable_nestedgroup_for_ad_set_access_control_restricted():
#     access_mode = "restricted"
#     validate_access_control_disable_auth(access_mode)
...
def validate_access_control_disable_auth(access_mode):
    delete_cluster_users()
    auth_setup_data = setup["auth_setup_data"]
    ...
    admin_user = auth_setup_data["admin_user"]
    token = login(admin_user, PASSWORD)
    disable_openldap(admin_user, token)
...
def validate_access_control_enable_nestedgroup(access_mode):
    delete_cluster_users()
    auth_setup_data = setup["auth_setup_data"]
    ...
    token = login(admin_user, PASSWORD)
    enable_openldap(admin_user, token)
    # By default nestedgroup feature is not enabled, so users under nestedgroups cannot be logged in
    allowed_principal_ids = []
...
def enable_openldap(username, token, expected_status=200):
    ...
    r = requests.put(CATTLE_AUTH_PROVIDER_URL, json={"ldapConfig": {
        ...
        "serviceAccountDistinguishedName": "cn=admin,dc=us-west-2,dc=compute,dc=internal",
        "groupDNAttribute": "entryDN",
        "groupMemberMappingAttribute": "member",
        "groupMemberUserAttribute": "entryDN",
        "groupNameAttribute": "cn",
        "groupObjectClass": "groupOfNames",
        "groupSearchBase": None,
        "groupSearchAttribute": "cn",
        "id": "openldap",
        "links": {
            "self": CATTLE_AUTH_PROVIDER_URL,
            "update": CATTLE_AUTH_PROVIDER_URL
        },
        "name": "openldap",
        ...
        "userSearchBase": "dc=us-west-2,dc=compute,dc=internal"
        },
        "username": username,
        "password": PASSWORD
